...
public class StashKitbotStatsHud extends HudElement {
    public static final HudElementInfo<StashKitbotStatsHud> INFO = new HudElementInfo<>(
        StashScannerStatsHud.GROUP,
        "stash-kitbot-stats",
        "Displays stash kitbot delivery counts and current request status.",
        StashKitbotStatsHud::new
    );
    ...
    private List<String> lines = List.of("Stash Kitbot", "Status: inactive");

    public StashKitbotStatsHud() {
        super(INFO);
    }
    ...
    private List<String> buildLines() {
        StashKitbot kitbot = Modules.get().get(StashKitbot.class);
        if (kitbot == null) return List.of("Stash Kitbot", "Status: missing");

        return StashKitbotStatsHudText.buildLines(kitbot.statsSnapshot());
    }
}
